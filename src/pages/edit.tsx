...
function NotesList({
  ...
  if (state.nav.type === 'draft') {
    ...
    draft = (
      <ListItem button onClick={() => openDraft()} selected>
        ...
        <ListItemText primary="Draft description" />
  ...
  } else if (state.auth.status === 'done' && Object.keys(state.notes).length < 1) {
    draft = (
      <ListItem button onClick={() => openDraft()}>
        <ListItemIcon>
          <NoteAddIcon />
        </ListItemIcon>
        <ListItemText primary="New description" />
      </ListItem>
    )
  } else if (Object.keys(state.notes).length == 1 && state.auth.status === 'done') {
    draft = (
      <ListItem>
        <ListItemIcon>
          <NoteAddIcon />
        </ListItemIcon>
        <ListItemText primary="Only one description allowed" />
      </ListItem>
    )
  } else {
    draft = (
      <ListItem>
        ...
        <ListItemText primary="Authenticate to create description" />
      </ListItem>
    )
  }
...
function AuthenticateScreen({ authenticate, state }: AuthenticateProps) {
  ...
  return state.status === 'done' ? (
    <>
      ...
      <Typography>Editing description of collection: {collectionID}</Typography>
    </>
  ) : (
    <>
      <Typography>
        Authenticate to edit your NFT's description
      </Typography>
      <Button
        color="primary"
...
type DraftScreenProps = {
  save: (title: string, text: string, address: string) => void
  status: DraftStatus
}
...
function DraftScreen({ save, status }: DraftScreenProps) {
  ...
  const { collectionID } = router.query
  console.log(collectionID);
  const address = String(collectionID);
  ...
  return (
    <>
      <Dialog
        ...
        <DialogTitle id="form-dialog-title">Save description</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
...
function NoteScreen({ note, save }: NoteScreenProps) {
  ...
  if (note.status === 'loading failed') {
    return <Typography>Failed to load description!</Typography>
  }

  if (note.status === 'init' || note.status === 'loading') {
    return <Typography>Loading description...</Typography>
  }
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  return {
      props: { 
        collectionID: context.query.collectionID, //pass it to the page props
      }
  }
}